...
export default function Criteria({ allData }: CriteriaProps) {
  ...
  const handleExportJSON = () => {
    ...
    a.href = url;
    a.download = `criteria_export_${Date.now()}.json`;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const importedCriteria = JSON.parse(content);
        if (Array.isArray(importedCriteria)) {
           for (const item of importedCriteria) {
             if (item.type === 'criterion') {
                await dataService.create({
                  ...item,
                  criterion_id: item.criterion_id || 'CRIT-IMPORT-' + Date.now() + Math.random(),
    ...
    <div className="page-content fade-in">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2">
           <button onClick={handleExportJSON} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors font-bold text-xs shadow-md">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingCriterion ? "แก้ไขเกณฑ์" : `เพิ่มเกณฑ์สำหรับ ${currentType === 'area' ? 'เขตพื้นที่' : currentType === 'classroom' ? 'ห้องเรียน' : 'ห้องน้ำ'}`}>
         <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            <div>
               <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">ชื่อหัวข้อเกณฑ์</label>
               <input 
                  type="text" required 
                  className="w-full px-4 py-2 border border-slate-200 rounded-xl text-slate-900 dark:bg-slate-800 dark:border-slate-700 dark:text-white"
                  value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})}
               />
            </div>
            <div>
               <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">คำอธิบาย</label>
               <textarea 
                  rows={2} required 
                  className="w-full px-4 py-2 border border-slate-200 rounded-xl text-slate-900 dark:bg-slate-800 dark:border-slate-700 dark:text-white"
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-4 mt-2">
               <h4 className="font-bold text-slate-900 dark:text-white mb-3">รูบริกการให้คะแนน (Rubrics)</h4>
               
               {[5, 4, 3, 2, 1].map(score => (
                  <div key={score} className="mb-3">
                     <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase">คะแนน {score}</label>
                     <textarea 
                        rows={2}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500/20 dark:bg-slate-800 dark:border-slate-700 dark:text-white"
            ...
            <div className="flex gap-3 pt-4 sticky bottom-0 bg-white dark:bg-slate-900">
               <button type="button" onClick={() => setModalOpen(false)} className="flex-1 px-4 py-2 text-slate-600 font-semibold rounded-xl hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 transition-colors">ยกเลิก</button>
